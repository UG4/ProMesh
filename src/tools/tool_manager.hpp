...
class ToolManager : public QObject {
	...
	public:
		...
		explicit ToolManager(QWidget* parent);
	 ...
	 *	*/
		...
		size_t num_tools() const			{return _registered_tools.size();}
		
		ITool* tool(size_t index) const		{return _registered_tools[index]._tool;}
	 ...
	 * \sa set_group_icon*/
		...
		size_t num_known_groups () const 				{return _known_groups.size();}
		const std::string& known_group (size_t i) const	{return _known_groups.at(i);}
	...
	private:
		...
		struct ToolEntry{
			ToolEntry(ITool* tool, int key, uint modifiers) :
				_tool(tool), _shortcut_key(key),
				_shortcut_modifiers(modifiers) {}
			ITool* _tool;
			int _shortcut_key;
			uint _shortcut_modifiers;
		};

		std::vector<ToolEntry> _registered_tools;
		std::vector<std::string> _known_groups;
		GroupIconMap _group_icon_map;
		QIcon _default_icon;
};
