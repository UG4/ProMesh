...
#include <string>
#include <vector>
#include "heightfield_interface.hpp"
...
public:
	// "public" sind funktionen, die von "au�en" sichtbar sind
	...
	~InterpolatedHeightfield() override {
		// das hier ist der Dekonstruktor, er sorgt daf�r dass alle Speicherbereiche freigegeben werden usw.
		// entspricht deinem "finish"
		cleanupDataMemory();
	}

	bool loads_from_file() override {
		// wir geben true zurück, da wir eine Datei laden möchten
		return true;
	}

	const char* file_name_extensions() override {
		// dateiendungen, damit nur diese im dateidialog angezeigt werden
		return "*.mesh";
	}

	bool initialize(const char* filename, number xMin, number yMin, number xMax, number yMax) override {
		return readmesh( filename ) == 0;
	}

	number height(number x, number y) override {
		...
		if(X==nullptr)	// Keine Daten bisher gelesen?
			return 0;
	...
	stop:
		...
		if(idx_d_i==0 && idx_d_j==0) {
			...
			z = interpoliere(Z[idx_c_i][idx_c_j], Z[idx_d_i][idx_d_j], alpha);	//interpoliere c und d (�ber x-koordinate)
		}else if(idx_d_j==0) {
			alpha = (y - Y[idx_b_i][idx_b_j]) / (Y[idx_d_i][idx_d_j] - Y[idx_b_i][idx_b_j]);
			z = interpoliere(Z[idx_b_i][idx_b_j], Z[idx_d_i][idx_d_j], alpha);	//interpoliere b und d (�ber y-koordinate)
		}else {
...
private:
	...
	int readmesh( const char* filename ) {
		...
		cleanupDataMemory();	// L�sche Daten, falls schon vorher mal verwendet wurde
...
public:
	...
	SquareHeightfield() = default;

	~SquareHeightfield() override = default;
